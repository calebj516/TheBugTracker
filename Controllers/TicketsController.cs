...
namespace TheBugTracker.Controllers
{
    public class TicketsController : Controller
    {
        ...
        public async Task<IActionResult> Edit(int id, [Bind("Id,Title,Description,Created,Updated,Archived,ProjectId,TicketTypeId,TicketPriorityId,TicketStatusId,OwnerUserId,DeveloperUserId")] Ticket ticket)
        {
            ...
            {
                BTUser btUser = await _userManager.GetUserAsync(User);
                Ticket oldTicket = await _ticketService.GetTicketAsNoTrackingAsync(ticket.Id); // taking a snapshot of what the data looks like before modifications are made
                ...
                Ticket newTicket = await _ticketService.GetTicketAsNoTrackingAsync(ticket.Id); // snapshot after modifications made to DB
                await _historyService.AddHistoryAsync(oldTicket, newTicket, btUser.Id);
